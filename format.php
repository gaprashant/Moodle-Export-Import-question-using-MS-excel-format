...
class qformat_xls extends qformat_default {
    ...
    public function readquestions($questions) {
		...
		{
			if(empty($v[0]) || empty($v[2]) || empty($v[3])) // qtype, qname, qtext
			{
				continue;
			}			
	...
	public function import_multichoice($question)
	{
		  ...
		  if(empty($question[4]) && empty($question[5]) && empty($question[6]) && empty($question[7]) && empty($question[8]))	
		  {	return false; }
		 ...
		 // $qo->questiontext = str_replace('&lt;br /&gt;','<br />',$qo->questiontext);		  
		  ...
		  $qo->defaultmark = (!empty($question[10])) ? $question[10] : 1; // default value hardcoded 
		  $qo->penalty = 	(!empty($question[11])) ? $question[11] : 0.33; // default value hardcoded 
	...
	public function import_truefalse($question)
	{
		 ...
		 {	return false; }
		  ...
		  // if options are blank
		  if(empty($question[4]) && empty($question[5]))	
		  {	return false; }
	...
	{		
		...
		if(empty($question[9]))	// if answer field is blank
		{	return false; }
